from reverse_timeseries_z_score import back_test_cal_weights
from Get_Two_Close_Price import back_test_get_data
...
df = back_test_get_data()
df.set_index(0, inplace=True)
print(back_test_cal_weights(df))
...
class backTesting:
    ...
    def execute(self):
        ...
        for idx, row in df.iterrows():
            # 매수 신호 확인
            self.buy_signal = np.where(row['low'] <= row['targetPrice'] <= row['high'], True,
                                       False)  # 목표가에 달성한 경우 목표가에 매수해 다음날 시가에 매도한 것으로 판단

            # 거래횟수 계산
            self.trade_count += 1 if self.buy_signal else 0

            # 수익률 계산
            self.ror = row['close'] / row[
                'targetPrice'] - self.fee if self.buy_signal else 1  # 다음날 시가와 당일 종가의 시간차이가 거의 없으므로 종가로 계산
